...
class HostileResponseManager:
    ...
    def __init__(self, storage_file: str = "hostile_responses_state.json"):
        ...
        # Паттерны агрессивных сообщений (скомпилированные для производительности)
        self.aggressive_patterns = [
            # Прямые оскорбления
            re.compile(r'\b(заткн[иу]|заткнись|заткни)\b', re.IGNORECASE),
            re.compile(r'\b(иди\s+нахуй|иди\s+в\s+жопу|иди\s+в\s+ж.*\b)\b', re.IGNORECASE),
            re.compile(r'\b(пошёл\s+нахуй|пошёл\s+в\s+жопу)\b', re.IGNORECASE),
            re.compile(r'\b(пошли\s+нахуй|пошли\s+в\s+жопу)\b', re.IGNORECASE),
            re.compile(r'\b(уёбок|уёбище|мудак|мудок|дурак|дебил|идиот|придурок|тупой|глупый|ничтожество)\b', re.IGNORECASE),
            re.compile(r'\b(лох|лошара|неудачник|неудачница)\b', re.IGNORECASE),
            re.compile(r'\b(слабоумный|тупоголовый|безмозглый)\b', re.IGNORECASE),
            
            # Команды молчать
            re.compile(r'\b(замолчи|молчи|молчать|тише|тише\s+там)\b', re.IGNORECASE),
            re.compile(r'\b(не\s+пиши|не\s+отвечай|не\s+комментируй)\b', re.IGNORECASE),
            re.compile(r'\b(не\s+мешай|не\s+вмешивайся)\b', re.IGNORECASE),
            
            # Проклятия и пожелания плохого
            re.compile(r'\b(чтоб\s+ты\s+сдох|чтоб\s+ты\s+сгнил|чтоб\s+ты\s+сгорел)\b', re.IGNORECASE),
            re.compile(r'\b(сдохни|умри|подыхай)\b', re.IGNORECASE),
            re.compile(r'\b(заткни\s+свою\s+жопу|закрой\s+свою\s+жопу)\b', re.IGNORECASE),
            
            # Уничижительные сравнения
            re.compile(r'\b(как\s+животное|как\s+скот|как\s+свинья)\b', re.IGNORECASE),
            re.compile(r'\b(хуже\s+животного|хуже\s+скот[ау])\b', re.IGNORECASE),
            re.compile(r'\b(примитивный|примитив)\b', re.IGNORECASE),
            
            # Сравнения с низшими существами
            re.compile(r'\b(как\s+аргонианин|как\s+хайм|как\s+каджит)\b', re.IGNORECASE),
            re.compile(r'\b(хуже\s+аргонианина|хуже\s+хайма|хуже\s+каджита)\b', re.IGNORECASE),
            
            # Уничижение интеллекта
            re.compile(r'\b(тупой\s+бот|глупый\s+бот|идиотский\s+бот)\b', re.IGNORECASE),
            re.compile(r'\b(бесполезный|бессмысленный)\b', re.IGNORECASE),
            re.compile(r'\b(отстой|хрень|гавно|дерьмо)\b', re.IGNORECASE),
            
            # Обвинения в бесполезности
            re.compile(r'\b(ты\s+никчёмный|ты\s+бесполезный)\b', re.IGNORECASE),
            re.compile(r'\b(ты\s+ничего\s+не\s+умеешь|ты\s+ничего\s+не\s+знаешь)\b', re.IGNORECASE),
            re.compile(r'\b(ты\s+никто|ты\s+ничто)\b', re.IGNORECASE)
        ]
